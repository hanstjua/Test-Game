...
public class Map : MonoBehaviour
{
    ...
    public GameObject HighlightPrefab { get; private set; }
    ...
    void Awake()
    {
        HighlightPrefab = Resources.Load("Prefabs/Maps/Highlight") as GameObject;
    ...
    {
        var highlightPosition = position + new Vector3(0, 0.16f, 0);

        if (_highlights.Exists(h => h.transform.position == highlightPosition))
        {
            return;
        }
        else
        {
            var highlight = Instantiate(HighlightPrefab, highlightPosition, Quaternion.identity);
